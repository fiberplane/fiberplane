...
export function useSSE<T = unknown>(
  ...
  const handlerMapRef = useRef<Map<string, (event: MessageEvent) => void>>(new Map());
  ...
  const shouldFilterEvent = useCallback((eventType: AgentEventType, parsedData: unknown): boolean => {
    // Only apply filtering if filterAdminEndpoints is true
    if (!optionsRef.current.filterAdminEndpoints) {
      return false;
    }
    
    // Only filter http_request events
    if (eventType !== "http_request") {
      return false;
    }
    
    // Check if the URL contains admin/db or admin/events
    if (typeof parsedData === 'object' && parsedData !== null && 'url' in parsedData && parsedData.url) {
      try {
        const urlString = parsedData.url.toString();
        return urlString.includes("/admin/db") || urlString.includes("/admin/events");
      } catch (e) {
