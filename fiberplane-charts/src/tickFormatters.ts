...
export function getFormatterForAxis(axis: Axis, kind: FormatterKind): TickFormatter {
    switch (kind) {
        case 'bytes':
...
function getScientificFormatterForUnits(largestUnit: ScientificUnit, minimumUnit: ScientificUnit): TickFormatter {
    return (value: number): string => {
        let unit = largestUnit;
        let adjusted = value * unit.multiplier();
