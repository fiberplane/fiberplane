---
title: "OTEL collector in Rust for Cloudflare Workers"
...
- Why process teletetry in `us-east-1` when you can can store them directly at the edge with Cloudflare’s D1?
- No YAML

We chose Rust for as it's a team-favorite tool inside of Fiberplane and like writing sharp tools
in the language such as our TUI log tailing CLI, [Fogwatch](/blog/fogwatch/).
...
It contains the workers-rs code, a Store implementation that targets 
Cloudflare's D1 database, and a WebSocket implementation that leverages 
Durable Objects for maintaining persistent connections.
...
Onece you've cloned the [repository](https://github.com/fiberplane/otel-worker),
...
To wrap things up, we’re well aware that Cloudflare may eventually launch an 
